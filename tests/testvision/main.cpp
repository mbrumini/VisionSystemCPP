...
private:
  ...
  void buildUi()
  {
    ...
    m_recipeEdit = new QLineEdit(configPanel);
    m_recipeEdit->setPlaceholderText("Ricetta attiva in Vision");
    ...
    form->addRow("Ricetta Vision", m_recipeEdit);
    ...
    connect(m_recipeEdit, &QLineEdit::textChanged, this, [this](const QString& text) {
  ...
  void updateMasterShape()
  {
    ...
    m_master = createShape(
      ...
      m_canvasHeight,
      m_canvasBackground);
    const QString suggestedRecipe = "test_" + m_shapeCombo->currentData().toString();
    if (m_recipeEdit && m_recipeEdit->text().isEmpty())
    {
      m_recipeEdit->setText(suggestedRecipe);
    }
    m_expectedRecipeId = m_recipeEdit ? m_recipeEdit->text().trimmed() : suggestedRecipe;
    updateScenarioLabel();
