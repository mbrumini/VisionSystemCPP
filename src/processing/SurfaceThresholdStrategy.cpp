  ...
  const SurfaceThresholdSettings& settings) const
{
  ...
  if (!result.blobs.empty())
  {
    ...
    if (mainBlob.contour.size() >= 5)
    {
      ...
      result.localization.xAxisStart = mainBlob.center - xDirection * axisLength;
      result.localization.xAxisEnd = mainBlob.center + xDirection * axisLength;
      result.localization.yAxisStart = mainBlob.center - yDirection * axisLength;
      result.localization.yAxisEnd = mainBlob.center + yDirection * axisLength;
